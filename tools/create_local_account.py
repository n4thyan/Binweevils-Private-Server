#!/usr/bin/env python3
"""Create fresh local/beta accounts for the legacy-compatible database path.

This tool is intentionally narrow. It supports guarded `local_` and `beta_`
usernames and creates the starter rows needed for login/bootstrap review:

- users
- buddylist

It does not import old users, old moderator names, old session keys, old
passwords, or production/private player data.
"""
...
USERNAME_PATTERN = re.compile(r"^(local|beta)_[A-Za-z0-9_]{3,24}$")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a fresh guarded Binweevils account for a disposable/local/beta database."
    )
    parser.add_argument("--database-url", required=True, help="MySQL/MariaDB URL for a disposable/local/beta database.")
    parser.add_argument("--username", required=True, help="Fresh account username, e.g. local_admin or beta_friend1.")
    parser.add_argument("--password-from-env", required=True, help="Environment variable containing the account password.")
...
def validate_plan(plan: LocalAccountPlan) -> list[str]:
    ...
    if not USERNAME_PATTERN.match(plan.username):
        errors.append("Username must start with local_ or beta_ and contain only letters, numbers, or underscores.")
...
def build_account_sql(plan: LocalAccountPlan, stored_password: str) -> str:
    ...
    return f"""-- Guarded account bootstrap generated by tools/create_local_account.py
-- Username: {plan.username}
-- Runtime rows created: users, buddylist
-- No old user, old session, old password, inventory, progress, or production data is imported.
...
def main() -> int:
    ...
    print("Account setup plan validated.")
