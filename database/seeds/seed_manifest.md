# Seed Extraction Manifest

Generated from `bwps.sql`.

This manifest is produced by `tools/extract_seed_tables.py`.

## Selected catalogue/reference tables

- `itemtype`: 13 INSERT statement(s)
- `itemtypets`: 1 INSERT statement(s)
- `appareltypes`: 1 INSERT statement(s)
- `gardenitemtype`: 1 INSERT statement(s)
- `seeds`: 1 INSERT statement(s)
- `puzzletypes`: 1 INSERT statement(s)
- `crosswords`: 1 INSERT statement(s)
- `questtasks`: 5 INSERT statement(s)

## Blocked player/runtime tables found

- `buddyalerts`: 1 blocked INSERT statement(s)
- `buddylist`: 1 blocked INSERT statement(s)
- `nest`: 1 blocked INSERT statement(s)
- `nestinfo`: 1 blocked INSERT statement(s)
- `users`: 1 blocked INSERT statement(s)
- `weevilitems`: 1 blocked INSERT statement(s)

## All INSERT tables found in source dump

- `appareltypes`: 1 INSERT statement(s)
- `buddyalerts`: 1 INSERT statement(s)
- `buddylist`: 1 INSERT statement(s)
- `crosswords`: 1 INSERT statement(s)
- `gardenitemtype`: 1 INSERT statement(s)
- `itemtype`: 13 INSERT statement(s)
- `itemtypets`: 1 INSERT statement(s)
- `nest`: 1 INSERT statement(s)
- `nestinfo`: 1 INSERT statement(s)
- `puzzletypes`: 1 INSERT statement(s)
- `questtasks`: 5 INSERT statement(s)
- `seeds`: 1 INSERT statement(s)
- `tycoonbusinesses`: 1 INSERT statement(s)
- `users`: 1 INSERT statement(s)
- `weevilitems`: 1 INSERT statement(s)

## Safety notes

Default seed extraction must not include old accounts, passwords, session keys, login keys, IP addresses, inventories, buddy lists, progress, moderation logs, or old staff/mod/celebrity identity records.
